@extends('layouts.master')

@section('title', 'category_blogs')

@section('pageName', 'Category_blog Create')

@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Category_blog Page</li>
@endsection

@section('content')
    <div class="mb-5">
        <a href="{{ route('category_blogs.index') }}" class="btn btn-sm btn-outline-primary">All Category_blogs</a>
    </div>

    <form action="{{ route('category_blogs.store') }}" method="post" enctype="multipart/form-data">
        @csrf

        {{-- two way to solve old data issue in form  --}}
        {{-- first use ( $category->name ?? '' )  its quick if-else --}}
        {{-- second  pass empty object in variable $category in Controller --}}
        {{-- second better --}}
        @include('dashboard.category_blogs._form')
    </form>
@endsection

{{-- @push('style')
    <link rel="stylesheet" href="{{ asset('dist/css/style.css') }}">
@endpush --}}
